...
static char *dump_cookie(SoupCookie *cookie) {
	...
	t = soup_date_to_time_t (cookie->expires);
	strftime(ts, 32, "%Y-%m-%dT%H:%M:%S%z", gmtime(&t));

	ret = g_strdup_printf ("HttpOnly:%s\tDomain:%s\tPath:%s\tSecure:%s\tExpires:%s\tExpiresTS:%ld\tName:%s\tValue:%s",
		 cookie->http_only ? "t" : "f",
		 cookie->domain,
		 cookie->path,
		 cookie->secure ? "t" : "f",
		 ts, (gulong)t,
