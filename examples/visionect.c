...
> meson configure _build -Dintrospection=enabled
> ninja -C _build
*/

/* building this visionect "test app"

> gcc visionect.c -o visionect -ggdb -L ../_build/libsoup/ -I ../libsoup/ -I ../_build/libsoup/ -lsoup-3.0 `pkg-config --libs --cflags glib-2.0 gobject-2.0`

running via debugger
> LD_LIBRARY_PATH=../_build/libsoup/ VSS_COOKIE_DB_ADDR= VSS_COOKIE_DB_PORT= VSS_COOKIE_DB_USER= VSS_COOKIE_DB_PASS= VSS_COOKIE_DB_NAME=  gdbserver :1234 visionect
...
void main() 
{
	...
	GUri *uri = g_uri_parse("http://google.com", SOUP_HTTP_URI_FLAGS, NULL);

	char *c = soup_cookie_jar_get_cookies(txt, uri, FALSE);
	printf("cookies for %s %s\n", g_uri_to_string(uri), c);
